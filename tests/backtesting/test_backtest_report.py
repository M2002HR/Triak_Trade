...
def test_report_json_and_telegram_summary() -> None:
    ...
    assert "period_pnl" in report_json
    assert "trade_outcome_summary" in report_json
    assert "per_signal_summary" in report_json
    assert "comparison_profile" in report_json
