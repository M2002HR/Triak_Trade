...
هیچ معامله‌ی واقعی‌ای انجام نمی‌دهد (`EXECUTION_MODE='live'` به‌صورت سخت مسدود است).
...
**مهم‌ترین یافته‌ها** (تفصیل در فایل ۰۸):
- یک مشکل **کارایی O(N²)** در شبیه‌سازی زنده‌ی هر پیام (`_update_live_simulation_state`).
- **ناهماهنگی conservative/optimistic** بین `total_pnl` و لیست تریدها در گزارش.
- **کارمزد/اسلیپیج/فاندینگ مدل نشده** (PnL خوش‌بینانه).
- چند مورد **تطبیق نماد ناهمگون** (`==` در مقابل `same_market_symbol`).
- مخرج **win_rate** شامل سیگنال‌های پر نشده و بریک‌ایون است.
