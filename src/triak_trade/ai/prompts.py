...
def build_telegram_signal_prompt(context: AIMessageContext) -> str:
    ...
    return (
        ...
        "Use entry_type=trigger when entry is explicitly described as an activation, "
        "breakout, trigger, stop-entry, فعال‌سازی, فعالسازی, or تریگر price. "
        "Use entry_type=limit only for one fixed entry price. Use entry_type=range whenever "
        "two distinct entry boundaries define an entry zone. "
